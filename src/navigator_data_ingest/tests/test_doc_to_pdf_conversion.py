...
def test_convert_doc_to_pdf():
    with open("src/navigator_data_ingest/tests/fixtures/sampe-01-for-conversion.doc", "rb") as file:
        doc_content = file.read()
    
    pdf_content = convert_doc_to_pdf(doc_content)

    converted_pdf = fitz.open(stream=pdf_content, filetype="pdf")
    expected_pdf = fitz.open("src/navigator_data_ingest/tests/fixtures/sampe-01-for-conversion.pdf")
    ...
    assert distance(all_text(converted_pdf), all_text(expected_pdf)) / len(all_text(expected_pdf)) < 0.06
    assert (len(all_text(converted_pdf)) - len(all_text(expected_pdf))) / len(all_text(expected_pdf)) < 0.01